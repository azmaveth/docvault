    ...
    async def get_library_docs(self, library_name: str, version: str = "latest") -> Optional[List[Dict[str, Any]]]:
        ...
        if version == "latest":
            ...
            if latest_lib and latest_lib["is_available"]:
                self.logger.info(f"Using latest known version {latest_lib['version']} for {library_name}")
        ...
        if not doc_url and version == "latest":
            self.logger.info(f"Couldn't find latest version URL, trying to find latest version for {library_name}")
            ...
            if not doc_url:
                self.logger.info(f"Trying common version patterns")
                # Try to resolve a URL with a common stable version 
                for fallback_version in ["stable", "1.0.0", "1.0", "0.1.0", "main", "master"]:
                    ...
                    if doc_url:
                        ...
                        self.logger.info(f"Found URL for fallback version {fallback_version}")
                        break
        ...
        loggers = [
            ...
            logging.getLogger("docvault.core.processor")
        ]
        ...
        try:
            ...
            if library_name in self.LIBRARY_LANGUAGES:
                ...
                if language in self.LANGUAGE_PATTERNS:
                    ...
                    url = pattern.replace("{library}", library_name).replace("{version}", version)
                    if await self.check_url_exists(url):
                        return url
