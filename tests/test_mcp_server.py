...
from docvault.mcp.server import create_server, run_server
...
def mock_get_scraper():
    ...
    scraper_mock.scrape_url = AsyncMock(return_value={
        "id": 123,
        "title": "Test Document",
...
def mock_read_markdown():
    with patch('docvault.core.storage.read_markdown', return_value="# Test Document\n\nThis is a test document content."):
        yield

@pytest.fixture
def mock_read_html():
    with patch('docvault.core.storage.read_html', return_value="<h1>Test Document</h1><p>This is a test document content.</p>"):
        yield
...
async def test_scrape_document_tool(mock_get_scraper):
    ...
    mock_get_scraper.scrape_url.assert_called_once_with("https://example.com/doc", depth=2)
...
async def test_lookup_library_docs_tool(mock_lookup_library_docs):
    ...
    assert len(result.content) == 1
    assert result.content[0].type == "text"
    assert "Documentation for test_library latest is available" in result.content[0].text
...
async def test_lookup_library_docs_not_found():
    ...
    with patch('docvault.mcp.server.lookup_library_docs', AsyncMock(return_value=[])):
        ...
        assert len(result.content) == 1
        assert result.content[0].type == "text"
        assert "Could not find documentation for nonexistent_library latest" in result.content[0].text
        assert result.metadata["success"] is False
        assert "Could not find documentation for nonexistent_library latest" in result.metadata["message"]
