...
class E2ETestRunner:
    ...
    def setup(self):
        ...
        if self.verbose:
            ...
            self.console.print(f"[green]Mock server running on port {self.mock_port}[/]")
    ...
    def teardown(self):
        ...
        for key in ["DOCVAULT_HOME", "DOCVAULT_DB_PATH", "STORAGE_PATH", 
                    "DOCVAULT_TEST_MODE", "NO_ANALYTICS", "URL_ALLOWED_DOMAINS",
        ...
        try:
            ...
            if "has_documents" in validation:
                for doc_title in validation["has_documents"]:
                    cursor.execute("SELECT id FROM documents WHERE title LIKE ?", (f"%{doc_title}%",))
            ...
            if "has_collections" in validation:
                for collection in validation["has_collections"]:
                    cursor.execute("SELECT id FROM collections WHERE name = ?", (collection,))
                    results[f"has_collection_{collection}"] = cursor.fetchone() is not None
    ...
    def validate_storage(self, validation: Dict[str, Any]) -> Dict[str, bool]:
        ...
        if "has_markdown_files" in validation:
            ...
            results["has_markdown_files"] = len(markdown_files) >= validation["has_markdown_files"]
        ...
        if "file_contains" in validation:
            for file_pattern, expected_content in validation["file_contains"].items():
                ...
                if files:
                    ...
                    results[f"file_contains_{file_pattern}"] = expected_content in content
                else:
    ...
    def run_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            for i, command in enumerate(test_case.commands):
                ...
                if test_case.expected_exit_codes and i < len(test_case.expected_exit_codes):
                ...
                if test_case.expected_outputs and i < len(test_case.expected_outputs):
                    ...
                    for expected in test_case.expected_outputs[i]:
                        if expected not in output:
                            if self.verbose:
                                self.console.print(f"[yellow]Command output:[/]\n{output}")
                            raise Exception(
            ...
            all_passed = all(validation_results.values()) if validation_results else True
            ...
            for result in results:
                if not result.success:
                    ...
                    if self.verbose and result.outputs:
                        self.console.print(f"   Last output: {result.outputs[-1][:200]}...")
...
@click.option('--list', '-l', 'list_only', is_flag=True, help='List tests without running')
