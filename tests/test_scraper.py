...
async def test_scrape_url_github_branch(mock_config, temp_dir, monkeypatch):
    ...
    monkeypatch.setattr(scraper, '_fetch_github_readme', AsyncMock(return_value=md_content))
    ...
    monkeypatch.setattr('docvault.core.processor.segment_markdown', lambda text: [('text', text)])
    monkeypatch.setattr('docvault.core.embeddings.generate_embeddings', AsyncMock(return_value=b'fake-embed'), raising=False)
    monkeypatch.setattr('docvault.db.operations.add_document_segment', lambda **kw: None)
    monkeypatch.setattr('docvault.db.operations.get_document', lambda doc_id: {'id': doc_id, 'url': 'https://github.com/owner/repo', 'title':'owner/repo', 'html_path':str(temp_dir/'gh.html'), 'markdown_path':str(temp_dir/'gh.md')})
...
async def test_fetch_github_readme_success(mock_config, monkeypatch):
    ...
    class Session:
        ...
        async def __aexit__(self, *args): pass
        def get(self, url, headers=None): return Context(MockResponse())
    monkeypatch.setenv("GITHUB_TOKEN", "fake-token")
    monkeypatch.setattr("docvault.config.GITHUB_TOKEN", "fake-token", raising=False)
    monkeypatch.setattr("aiohttp.ClientSession", lambda *a, **kw: Session(), raising=True)
...
async def test_fetch_github_readme_error(mock_config, monkeypatch):
    ...
    class MockResponse:
        status = 500
        async def json(self): raise Exception("error")
    class Context:
        def __init__(self, resp): self.resp = resp
        async def __aenter__(self): return self.resp
        async def __aexit__(self, *args): pass
    ...
    monkeypatch.setenv("GITHUB_TOKEN", "fake")
    monkeypatch.setattr("docvault.config.GITHUB_TOKEN", "fake", raising=False)
    monkeypatch.setattr("aiohttp.ClientSession", lambda *a, **k: Session(), raising=True)
    result = await scraper._fetch_github_readme("owner", "repo")
    assert result is None
...
async def test_scrape_readthedocs_site(mock_config, temp_dir, monkeypatch):
    ...
    monkeypatch.setattr('docvault.db.operations.add_document_segment', lambda **kw: None)
    monkeypatch.setattr('docvault.db.operations.get_document',
                        lambda doc_id: {'id': doc_id, 'url': 'https://example.readthedocs.io',
                                       'title': 'Docs', 'html_path': str(temp_dir / 'docs_html.html'),
                                       'markdown_path': str(temp_dir / 'docs_md.md')})
    # Patch processor and embeddings
    monkeypatch.setattr('docvault.core.processor.segment_markdown', lambda md: [('h1', 'Header'), ('text', 'Paragraph')])
    monkeypatch.setattr('docvault.core.embeddings.generate_embeddings', AsyncMock(return_value=b'\x00'), raising=False)
    # Invoke scraper
...
async def test_scrape_mkdocs_site(mock_config, temp_dir, monkeypatch):
    ...
    monkeypatch.setattr('docvault.db.operations.add_document_segment', lambda **kw: None)
    monkeypatch.setattr('docvault.db.operations.get_document',
                        lambda doc_id: {'id': doc_id, 'url': 'https://mk.example.com',
                                       'title': 'MK', 'html_path': str(temp_dir / 'mk.html'),
                                       'markdown_path': str(temp_dir / 'mk.md')})
    # Patch processor and embeddings
    monkeypatch.setattr('docvault.core.processor.segment_markdown', lambda md: [('h2', 'Section')])
    monkeypatch.setattr('docvault.core.embeddings.generate_embeddings', AsyncMock(return_value=b'\x00'), raising=False)
    # Invoke scraper
...
async def test_process_github_repo_structure(mock_config, temp_dir, monkeypatch):
    ...
    class RepoResp:
        status = 200
        async def json(self): return {"default_branch": default_branch}
    class TreeResp:
        status = 200
        async def json(self): return {"tree": [{"path": path, "type": "blob"}]}
    class ContentResp:
        status = 200
        async def json(self): return {"content": encoded, "encoding": "base64"}
    class Context:
        def __init__(self, resp): self.resp = resp
        async def __aenter__(self): return self.resp
        async def __aexit__(self, *args): pass
    class Session:
        ...
        def get(self, url, headers=None):
            if url == repo_api: return Context(RepoResp())
            if url == tree_api: return Context(TreeResp())
            if url.endswith(f"/{path}"): return Context(ContentResp())
    ...
    monkeypatch.setattr("docvault.core.processor.segment_markdown", lambda md: [("text", md)], raising=False)
    monkeypatch.setattr("docvault.core.embeddings.generate_embeddings", AsyncMock(return_value=b"emb"), raising=False)
...
async def test_docs_pagination_and_nav(mock_config, temp_dir, monkeypatch):
    ...
    monkeypatch.setattr('docvault.db.operations.add_document_segment', lambda **kw: None)
    monkeypatch.setattr('docvault.db.operations.get_document', lambda doc_id: {'id': doc_id})
    monkeypatch.setattr('docvault.core.processor.segment_markdown', lambda md: [('text', md)], raising=False)
    monkeypatch.setattr('docvault.core.embeddings.generate_embeddings', AsyncMock(return_value=b'emb'), raising=False)
...
async def test_github_wiki_page_scraping(mock_config, temp_dir, monkeypatch):
    ...
    monkeypatch.setattr('docvault.db.operations.add_document_segment', add_seg)
    monkeypatch.setattr('docvault.db.operations.get_document', lambda doc_id: {'id': doc_id})
    monkeypatch.setattr('docvault.core.processor.segment_markdown', lambda md: [('text', 'Some wiki content')], raising=False)
    monkeypatch.setattr('docvault.core.embeddings.generate_embeddings', AsyncMock(return_value=b'emb'), raising=False)
    # Scrape wiki page
    doc = await scraper.scrape_url(url)
    assert doc['id'] == 321
...
async def test_github_readme_scraping(mock_config, temp_dir, monkeypatch):
    ...
    monkeypatch.setattr('docvault.core.scraper.WebScraper._fetch_github_readme', AsyncMock(return_value=md_content))
    monkeypatch.setattr('docvault.core.scraper.WebScraper._process_github_repo_structure', AsyncMock(), raising=False)
    # Patch storage
    def fake_save_html(content, url_arg):
