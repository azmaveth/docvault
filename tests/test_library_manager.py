...
async def test_search_doc_url(mock_config):
    ...
    with patch('docvault.config.BRAVE_API_KEY', 'fake-api-key'):
        ...
        # Apply the patch
        with patch.object(LibraryManager, 'search_doc_url', mock_search_doc_url), \
...
async def test_get_library_docs_cached(mock_config, test_db):
    ...
    from docvault.core.library_manager import LibraryManager
    from docvault.db.operations import add_library, add_document
    ...
    with patch('docvault.db.operations.get_library_documents', return_value=[{"id": doc_id, "title": "pytest 7.0.0"}]):
...
async def test_get_library_docs_latest_version(mock_config, test_db):
    ...
    with patch('docvault.db.operations.get_latest_library_version', 
               return_value={"id": 2, "version": "7.0.0", "is_available": True}):
        
        # Mock operations.get_library_documents
        with patch('docvault.db.operations.get_library_documents', 
...
async def test_find_latest_version_with_search(mock_config):
    ...
    # Apply the patch
    with patch.object(LibraryManager, 'find_latest_version_with_search', mock_find_latest_version):
